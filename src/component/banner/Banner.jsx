...
const Banner = () => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      const requestedMovie = await axiosBaseURl.get(
  ...
  const truncate = (string, cutFromThisChar) => {
    return (
      // we are checking string length first and then if char is more than 150 word_count then it will cut the rest string and show in last ... , otherwise it will show the normal string
      string?.length > cutFromThisChar
        ? string.substr(0, cutFromThisChar - 1) + "...."
        : string
    );
  };
